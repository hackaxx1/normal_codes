...
def render():
    ...
    if "secilen_proje" not in st.session_state:
        st.session_state.secilen_proje = data[0]
    ...
    with col1:
        ...
        if st.session_state.clicked_button_task != None:
            st.session_state.secilen_proje = st.selectbox("Bir proje seçin:", proje_adlari, index = proje_adlari.index(st.session_state.clicked_button_task["ad"])["ad"])
            print(proje_adlari.index(st.session_state.clicked_button_task["ad"]))
        else:
            st.session_state.secilen_proje = st.selectbox("Bir proje seçin:", proje_adlari)
            st.session_state.secilen_proje = next((item for item in data if item["ad"] == st.session_state.secilen_proje), None)["ad"]
    with col2:
